...
def get_labels_mapping(labels):
    labels = set(labels)
    semantic2id = {k:v for v,k in enumerate(list(labels))}
    id2semantic = {v:k for k,v in semantic2id.items()}
    return semantic2id, id2semantic

def get_y_y_hat(semantic2id):
    def f(gt, pred):
        y = np.zeros(len(semantic2id))
        y_hat_raw = np.zeros(len(semantic2id))
        for g in gt:
            y[semantic2id[g]] = 1
        for p in pred:
            y_hat_raw[semantic2id[p]] = 1.0
        return y, y_hat_raw
    return f

def get_y_y_hat_matrix(semantic2id, gts, preds):
    y_matrix = []
    y_hat_raw_matrix = []
    _func = get_y_y_hat(semantic2id)
    assert len(gts) == len(preds)
    for g, p  in zip(gts, preds):
        y, y_hat_raw = _func(g, p)
        y_matrix.append(y)
        y_hat_raw_matrix.append(y_hat_raw)
    return np.array(y_matrix), np.array(y_hat_raw_matrix)
